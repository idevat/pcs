...
When a command function passes the readability test (the reader *can* understand
the main function without reading the helpers), extract private helpers within
the same module. Follow these conventions:
...
(e.g. `_validate_elements_to_remove`).
...
[testing-library-commands.md](testing-library-commands.md).
...
the guidance in `testing-library-commands.md` over copying from arbitrary
existing tests.
...
- **Recent `pcs/lib/commands/`** implementations over older ones — good
  references: `pcs/lib/commands/cib.py` (`element_description_set`,
  `element_description_get`, `rename_node`),
  `pcs/lib/commands/remote_node.py` (`get_resource_ids`). See the reference
  table in `docs/architecture.md` for what each demonstrates.
