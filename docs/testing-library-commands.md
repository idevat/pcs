...
This document describes testing frameworks and patterns for pcs.

## Test Tiers

Tests are organized into two tiers with different scope:

- **Tier0** (`pcs_test/tier0/`) — tests library commands and internal modules
  directly. Uses the `get_env_tools` mock framework. Fast, no external
  dependencies. Covers business logic comprehensively.
...
**When to use which:**
- Tier0 for thorough testing of library command logic (all edge cases, report
  combinations, force patterns)
- Tier1 for verifying CLI integration — that the command is routable, arguments
  are parsed correctly, and the library command is invoked and produces the
  expected effect
...
# Testing Library Commands (Tier0)

This section describes the testing framework and patterns used for integration
tests of library commands (`pcs/lib/commands/`).

## Running Tests

Tests are run via `pcs_test/suite.py`. The bundled packages directory must be
on `PYTHONPATH`:

```bash
source /root/projects/pcs.venv/bin/activate
export PYTHONPATH=/root/projects/pcs/pcs_bundled/packages
python pcs_test/suite.py <test_spec> [<test_spec> ...]
```

Each `<test_spec>` is a dotted path to a **test module** (or class/method
within a module):

```bash
# Run all tests in a module
python pcs_test/suite.py pcs_test.tier0.common.reports.test_messages

# Run a single test class
python pcs_test/suite.py pcs_test.tier0.common.reports.test_messages.CibUpgradeSuccessful

# Run a single test method
python pcs_test/suite.py pcs_test.tier0.common.reports.test_messages.CibUpgradeSuccessful.test_success

# Run multiple modules
python pcs_test/suite.py pcs_test.tier0.lib.cib.test_something pcs_test.tier0.common.reports.test_messages
```

**Note:** The minimum granularity is a module — a bare package path like
`pcs_test.tier0.lib.commands.cluster` is not sufficient.

To run the entire tier0 suite: `make tests_tier0` (or `python pcs_test/suite.py
--tier0`).

## Test Framework Overview

Library command tests use a custom test environment (`command_env`) that mocks
external dependencies (CIB loading, cluster state, node communication) while
running the actual library command logic. This allows integration-style testing
of complete commands without a live cluster.
...
`optional_in_conf` is a generic `put_or_replace` for any optional section
under `<configuration>`. Use it when no named shortcut exists for the section
you need. If a test needs **two different** optional sections simultaneously
(e.g. both `fencing_topology` and `acls`), use the named shortcuts — each
keyword argument can only appear once.

### Pushing modified CIB

```python
# Expect CIB push with these section contents
...
### Non-live CIB (file-based, `-f` flag)
...
self.config.env.set_corosync_conf_data(
    _corosync_conf("node1", "node2")
)
```

### Testing `-f` mode (no corosync.conf)

When testing a command that gates corosync access on `env.is_cib_live`,
using `-f` mode (`set_cib_data`) means the command should skip corosync
access — no `set_corosync_conf_data` call is needed:
...
Note: `assert_raise_library_error` verifies that `LibraryError` is raised.
Reports are checked separately via `assert_reports`.

### Force pattern (error → warning with force flag)
...
When a command has multiple independent effects (e.g. renames in locations,
rule expressions, fencing levels), **consolidate into fewer, broader tests**
rather than writing a separate test class for each effect:
...
- **One "no match" test** that populates CIB with items that *look* related
  ...
  type). This verifies that none of the rename mechanisms activate on
  irrelevant data.
...
Report messages (`pcs/common/reports/messages.py`) have their own tests in
`pcs_test/tier0/common/reports/test_messages.py`.

Each message class needs a test class with the **same name**, inheriting from
`NameBuildTest`:
...
Tier1 tests run actual `pcs` commands via subprocess and verify their effects.
...
Commands that call `ensure_live_env(env)` reject both `-f` and
`--corosync_conf`. Their tier1 tests can't do end-to-end file-based
verification. Instead, they test:
...
variant), write a fixture to a temp file and verify the result after running:
...
When adding a new test file, remember to add it to `pcs_test/Makefile.am`.
Similarly, new source files under `pcs/` must be listed in `pcs/Makefile.am`.
Both Makefiles list every `.py` file individually. Missing this step won't
cause test or lint failures — it causes packaging/distribution failures that
surface late.
