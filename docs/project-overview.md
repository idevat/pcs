...
- Legacy Ruby daemon still runs alongside Tornado daemon
...
APIv2 is replacing NodeCommunicator (provided by `LibraryEnvironment`).

**Why:**
- Enables async request processing
- Removes `pycurl` dependency
- Modern HTTP API patterns

**Current state:**
- APIv2 implementation exists and is being expanded
- Old sync APIs still present
- Migration ongoing, not complete

## Development Philosophy

See [development-principles.md](development-principles.md) for detailed guidelines.
...
- **Pragmatic evolution** - we improve architecture while keeping the app
  functional, so not all code reflects future vision
